...
// Feature flags: set to true to enable navigation items
const NAVIGATION_ENABLED = false; // Controls Demo & Pricing nav items
const FEATURES_ENABLED = false;   // Controls Features dropdown menu
...
export default function Navigation() {
  ...
  // Navigation items - controlled by NAVIGATION_ENABLED flag
  const navItems = NAVIGATION_ENABLED ? [
    { name: 'Demo', href: '/#demo' },
    { name: 'Pricing', href: '/pricing' },
  ] : [];

  // Feature items - controlled by FEATURES_ENABLED flag
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center">
            {/* Navigation Items (controlled by NAVIGATION_ENABLED) */}
            {NAVIGATION_ENABLED && (
          ...
          <div className="md:hidden flex items-center space-x-3">
            ...
            {/* Mobile Menu Button - only show when navigation items exist */}
            {(NAVIGATION_ENABLED || FEATURES_ENABLED) && (
              <button
        ...
        {/* Mobile Menu - only show when navigation items exist */}
        {isMobileMenuOpen && (NAVIGATION_ENABLED || FEATURES_ENABLED) && (
          <div className="md:hidden bg-white/95 backdrop-blur-md border-t border-gray-200/20">
            <div className="px-2 pt-2 pb-3 space-y-1">
              {/* Navigation Items (controlled by NAVIGATION_ENABLED) */}
              {NAVIGATION_ENABLED && navItems.map((item) => (
                <Link
                  key={item.name}
                  href={item.href}
              ...
              {/* Mobile Features Section (controlled by FEATURES_ENABLED) */}
              {FEATURES_ENABLED && (
                <div className="pt-2">
                  <button
              ...
              {/* Optional: Simple Login link for quick access */}
